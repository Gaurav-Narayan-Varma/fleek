...
const ftdBuyerSets = Object.values(ftdBuyerData).map((country) => {
    return {
        label: country.cc,
        data: Array.from({length: 300}, (_, index) => {
            return country.time_to_deliver_list[index] > 0 ? country.time_to_deliver_list[index] : 0
        }),
        backgroundColor: getRandomColor(),
    }
})
...
export default function FTDChart() {
    ...
    const options = {
        plugins: {
            title: {
                display: true,
                text: `Time from Fulfillment to Delivery by ${option==='buyer'?'Buyer':'Vendor'} Country`,
            },
            legend: {
                display: false
            },
        ...
        scales: {
            ...
            title: {
                display: true,
                text: 'Hours from Fulfillment to Delivery'
            }
            },
            y: {
            stacked: true,
