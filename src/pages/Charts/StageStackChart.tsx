...
export default function StageStackChart() {
  ...
  const dsetsf =
    option === "buyer"
      ? dsets.concat(stageBuyerSets)
      : dsets.concat(stageVendorSets);
  ...
  return (
    <div id="time-in-stage">
      <div
        id="chart"
        ref={addToRef}
        className="opacity-0 transition-all translate-x-full duration-1000 max-w-6xl flex justify-center border border-stone-200 rounded-xl"
      >
        <div
          id="top-left"
          className="w-full pt-2 bg-white basis-4/6 rounded-xl"
        >
          <div className="text-stone-700 font-bold text-xl text-center mb-2">
            Average Hours Each Stage by{" "}
            <select
              onChange={(e) => setOption(e.target.value)}
              className="text-stone-700 bg-white rounded-md border border-stone-200"
            >
        ...
        {option === "buyer" ? (
          ...
          >
            ...
            <div
              id="bullet"
              className='opacity-1 transition-all duration-1000 delay-300 before:content-["🧾"] before:absolute before:-ml-7 before:mt-2.5 basis-1/3'
            >
              Less time to confirm order correlates with more orders for top
              three markets
            </div>
            <div
              id="bullet"
              className='opacity-1 transition-all duration-1000 delay-500 before:content-["🛠️"] before:absolute before:-ml-7 before:mt-2.5 basis-1/3'
            >
              'Failure' packages in UK and ROW markets higher, significant area
              to improve
            </div>
          </div>
        ) : (
          ...
          >
            ...
            <div
              id="bullet"
              className='opacity-1 transition-all duration-1000 delay-500 before:content-["🏆"] before:absolute before:-ml-7 before:mt-2.5 basis-1/3'
            >
              UK has 2nd lowest transit/confirmation times, ~0 failure rate, and
              lowest time
            </div>
          </div>
        )}
      ...
      <div
        id="chart"
        ref={addToRef}
        className="opacity-0 transition-all translate-x-full duration-1000 mt-1 max-w-6xl flex justify-center border bg-white border-stone-200 rounded-xl"
      >
        {option === "buyer" ? (
          <table className="border-collapse text-sm text-black border border-stone-600 self-center my-2">
            <thead>
              <tr>
                <th className="border text-left border-stone-400 font-semibold">
                  Country
                </th>
                <th className="border text-left border-stone-400 font-semibold">
                  Label purchased
                ...
                return (
                  <tr>
                    ...
                    <td className="border border-stone-400">
                      {country.label_purchased} hours
                    </td>
                    <td className="border border-stone-400">
                      {country.label_printed} hours
                    </td>
                    <td className="border border-stone-400">
                      {country.confirmed} hours
                    </td>
                    <td className="border border-stone-400">
                      {country.in_transit} hours
                    </td>
                    <td className="border border-stone-400">
                      {country.out_for_delivery} hours
                    </td>
                    <td className="border border-stone-400">
                      {country.attempted_delivery} hours
                    </td>
                    <td className="border border-stone-400">
                      {country.delivered} hours
                    </td>
                    <td className="border border-stone-400">
                      {country.failure} hours
                    </td>
                  </tr>
                );
              })}
          ...
          <table className="border-collapse text-sm text-black border border-stone-600 self-center my-2">
            <thead className="font-thin">
              <tr>
                <th className="border border-stone-400 font-semibold">
                  Country
                </th>
                <th className="border border-stone-400 font-semibold">
                  Label purchased
