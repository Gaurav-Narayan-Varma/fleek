...
for (let key in TtfBuyerData) {
  for (
    let i = 0;
    i <
    TtfBuyerData[key as keyof typeof TtfBuyerData].time_to_fulfill_list.length;
    i++
  ) {
    TtfBuyerData[key as keyof typeof TtfBuyerData].time_to_fulfill_list[i] =
      Math.floor(
        TtfBuyerData[key as keyof typeof TtfBuyerData].time_to_fulfill_list[i]
      );
  }
  ...
  const max_value = Math.max(
    ...TtfBuyerData[key as keyof typeof TtfBuyerData].time_to_fulfill_list
  );
...
const ttfBuyerSets = Object.values(TtfBuyerData).map((country) => {
  return {
    ...
    data: Array.from({ length: 100 }, (_, index) => {
      return country.time_to_fulfill_list[index] > 0
        ? country.time_to_fulfill_list[index]
        : 0;
    }),
...
for (let key in TtfVendorData) {
  for (
    let i = 0;
    i <
    TtfVendorData[key as keyof typeof TtfVendorData].time_to_fulfill_list
      .length;
    i++
  ) {
    TtfVendorData[key as keyof typeof TtfVendorData].time_to_fulfill_list[i] =
      Math.floor(
        TtfVendorData[key as keyof typeof TtfVendorData].time_to_fulfill_list[i]
      );
  }
...
export default function TTFByHourChart() {
  ...
  return (
    <div
      id="a1q1a"
      className="text-stone-700 font-bold text-xl text-center mb-2"
    >
      Appendix A: Time to Fulfill Distribution by{" "}
      <select
